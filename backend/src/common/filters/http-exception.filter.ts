  ...
  private resolve(exception: unknown): { status: number; body: ApiErrorResponse } {
    if (exception instanceof HttpException) {
      ...
      if (typeof payload === 'object' && payload !== null) {
        const { message, errors } = payload as { message?: string; errors?: FieldError[] }
        return {
          status,
          body: {
            success: false,
    ...
    if (exception instanceof Prisma.PrismaClientKnownRequestError) {
      if (exception.code === 'P2002') {
        return {
          ...
          body: { success: false, message: 'Dữ liệu đã tồn tại, vui lòng kiểm tra lại' },
        }
      }
    ...
    return {
      ...
      body: { success: false, message: 'Đã có lỗi xảy ra ở máy chủ, vui lòng thử lại sau' },
    }
  }
}
